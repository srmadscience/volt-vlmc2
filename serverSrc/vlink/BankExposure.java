...
public class BankExposure extends VoltProcedure {
...
//    
    public static final SQLStmt getInbound = new SQLStmt("SELECT sum(exposure) exposure,  sum(how_many) how_many  "
    ...
    public VoltTable[] run(String cycleDate, String cycleNumber) throws VoltAbortException {
        ...
        VoltTable[] banks = voltExecuteSQL();
        ...
        while (banks[0].advanceRow()) {
            String bname = banks[0].getString("BNAME");
            voltQueueSQL(getInbound, cycleDate, cycleNumber, bname);
            ...
            if (results[0].advanceRow()) {
                inboundValue = results[0].getLong("EXPOSURE");
                inboundCount = results[0].getLong("HOW_MANY");
            }

            if (results[1].advanceRow()) {
                outboundValue = results[1].getLong("EXPOSURE");
                outboundCount = results[1].getLong("HOW_MANY");
            }
